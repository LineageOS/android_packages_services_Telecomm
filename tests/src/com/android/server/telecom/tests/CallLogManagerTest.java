...
public class CallLogManagerTest extends TelecomTestCase {
    ...
    public void testDontLogCallsToSensitivePhoneNumber() {
        ...
        TelephonyManager mockTelephonyManager = (TelephonyManager) mContext.getSystemService(Context.TELEPHONY_SERVICE);
    ...
    private void verifyNoInsertionIfFileExists() {
        ...
        try {
            Thread.sleep(TEST_TIMEOUT_MILLIS);
            verify(mContentProvider, sensiblePNFile.exists()?never():atLeastOnce()).insert(any(String.class),
                    any(Uri.class), any(ContentValues.class));
        } catch (android.os.RemoteException e) {
            fail("Remote exception occurred during test execution");
        } catch (InterruptedException e) {
